      ...
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-8 mb-12">
          ...
          <div className="md:col-span-2 lg:col-span-1 mb-6 md:mb-0">
            <Logo /> {/* Make sure Logo component text color is adaptable or white here */}
            <p className="mt-3 text-sm">
              Canada's home for rental properties.
            </p>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {footerLinks.company.map(link => (
                <li key={link.label}><Link to={link.href} className="hover:text-white text-sm">{link.label}</Link></li>
              ))}
            </ul>
          </div>
          <div>
          ...
          <div className="lg:col-span-1"> {/* On large screens, it takes 1 of 5 cols. Adjust as needed. */}
             <h5 className="font-semibold text-white mb-3">Popular Cities</h5>
             <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-x-4"> {/* For cities, maybe 1 col on lg */}
                {footerLinks.cities.slice(0, 4).map(link => ( // Example: show first 4 directly
                  <div key={link.label} className="mb-1">
                    <Link to={link.href} className="hover:text-white text-sm">{link.label}</Link>
                  </div>
                ))}
        ...
        <div className="border-t border-slate-700 pt-8 text-center md:text-left">
          <div className="flex flex-col md:flex-row justify-between items-center">
            <p className="text-sm mb-4 md:mb-0">
              &copy; {currentYear} Rentals.ca. All rights reserved. A <a href="https://gotarent.com" target="_blank" rel="noopener noreferrer" className="underline hover:text-white">gotarent.com</a> network site.
            </p>
          ...
          <p className="text-xs mt-4 text-slate-400 text-center">
            Disclaimer: Rentals.ca is not responsible for the accuracy of information provided by landlords or users. 
            Always verify information independently and take precautions to avoid scams.
          </p>
        </div>
      </div>
    </footer>
